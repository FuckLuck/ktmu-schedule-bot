...
- ⚡ **Мгновенная отдача расписания (Instant Zero-Latency Cache)**:
  - **API-First парсинг**: исключены лишние сетевые задержки и медленные попытки чтения пустого HTML React-SPA колледжа.
  - **Оперативный RAM Hot Cache**: повторные запросы отдаются за 0 мс прямо из оперативной памяти.
  - **Фоновый предпрогрев (Pre-warming)**: при выборе группы и при запуске бота расписание на сегодня, завтра и неделю автоматически загружается и кэшируется в фоне.
- 👑 **Панель администратора (`/admin`, ID: 870396858)**:
  - 📊 Просмотр статистики в реальном времени (пользователи, активные группы, подписчики).
  - 👥 Рейтинг самых популярных групп среди студентов.
  - 📋 Список последних зарегистрированных пользователей с никами и временем активности.
  - 📢 Массовая рассылка (Broadcast) с защитой от блокировок и отчетом о доставке.
  - ⚡ Кнопка принудительного прогрева кэша всех групп и сброса кэша.
- 💬 **Поддержка групповых чатов (бесед групп/классов)**:
  - Бота можно добавлять в учебные беседы студентов.
  - При добавлении бот приветствует участников и предлагает администратору выбрать группу колледжа.
  - **Каждое утро ровно к 08:00** бот автоматически присылает расписание на день прямо в чат группы!
  - Команды в чате: `/today`, `/tomorrow`, `/week`, `/set_group` (настройка учебной группы доступна только администраторам чата).
  - Никаких мешающих Reply-клавиатур в общем чате — только аккуратные сообщения и команды.
- 🛡️ **Встроенный антиспам (Throttling Middleware)**: Защита от флуда и DDOS. Ограничивает частые запросы (сообщения и нажатия кнопок) с защитой от циклического спама ответами и автоматической очисткой памяти.
